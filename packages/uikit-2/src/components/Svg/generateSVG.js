...
fileNames.forEach(name => {
  if (name !== 'Warning2.tsx') {
    ...
    console.log(name)
    const svgString = data.match(/<Svg (.|\n)* <\/Svg>/m)[0].replace('<Svg', "<svg xmlns='http://www.w3.org/2000/svg'").replace('</Svg>', '</svg>').replace('{...props}', '').replace(/_\d+:\d+/g, '')
      .replace(/"/g, "'")
      .replace(/%/g, '%25')
      .replace(/#/g, '%23')
      .replace(/</g, '%3C')
      .replace(/>/g, '%3E')
    const alreadyFilled = svgString.includes('fill')
    const template = `
