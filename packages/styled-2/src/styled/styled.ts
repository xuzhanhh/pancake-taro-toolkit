...
type Options = {
  ...
  class: string
  vars?: {
    [key: string]: [string | number | ((props: unknown) => string | number), string | void]
  }
}

// Workaround for rest operator
const restOp = (obj: { [key: string]: any }, keysToExclude: string[]): { [key: string]: any } =>
  Object.keys(obj)
...
function styled<T>(tag: [T] extends [{ className?: string }] ? React.ComponentType<T> : never): ComponentStyledTag<T>
function styled<TName extends keyof JSX.IntrinsicElements>(tag: TName): HtmlStyledTag<TName>
...
type StyledComponent<T> = StyledMeta &
  ([T] extends [React.FunctionComponent<any>] ? T : React.FunctionComponent<T & { as?: React.ElementType }>)

type StaticPlaceholder = string | number | CSSProperties | StyledMeta

type HtmlStyledTag<TName extends keyof JSX.IntrinsicElements> = <TAdditionalProps = {}>(
...
>(
  ...
  ...exprs: TrgProps extends { style?: React.CSSProperties }
    ? Array<StaticPlaceholder | ((props: NoInfer<OwnProps & TrgProps>) => string | number)>
    : StaticPlaceholder[]
